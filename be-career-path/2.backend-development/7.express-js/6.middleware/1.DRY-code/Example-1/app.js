const express = require('express');
const morgan = require('morgan');
const bodyParser = require('body-parser');
const app = express();

const PORT = process.env.PORT || 4001;

app.use(express.static('public'));

const jellybeanBag = {
  mystery: {
    number: 4
  },
  lemon: {
    number: 5
  },
  rootBeer: {
    number: 25
  },
  cherry: {
    number: 3
  },
  licorice: {
    number: 1
  }
};
// // I
// const logRequest = (verb) => {
//   console.log(`${verb} Request Received`);
// }

//II
// app.use((req, res, next) => {
//   console.log(`${req.method} Request Received`);
//   //III
//   next();
// });

//VII using morgan
// app.use(morgan('tiny'));
app.use(morgan('dev')); //outputs more information.

// X adding body-parser middleware intead of our own
app.use(bodyParser.json);

//IV
app.use('/beans/:beanName', (req, res, next) => {
  const beanName = req.params.beanName;
  if (!jellybeanBag[beanName]) {
    // console.log('Response Sent');
    // return res.status(404).send('Bean with that name does not exist');
    //error middleware:
    const err = new Error('Bean with that name does not exist');
    err.status = 404;
    return next(err);
  };
  req.bean = jellybeanBag[beanName];
  req.beanName = beanName;
  next();
});

//V
// app.use(['/beans/', '/beans/:beanName'], (req, res, next) => {
//     let bodyData = '';
//     req.on('data', (data) => {
//         bodyData += data;
//     });
//     req.on('end', () => {
//         if (bodyData) {
//             req.body = JSON.parse(bodyData);
//         }
//         next();
//     });
// });

//IV
// const bodyParser = (req, res, next) => {
//   let bodyData = '';
//   req.on('data', (data) => {
//     bodyData += data;
//   });
//   req.on('end', () => {
//     if (bodyData) {
//       req.body = JSON.parse(bodyData);
//     }
//     next();
//   });
// }

app.get('/beans/', (req, res, next) => {
  // logRequest('GET');
  res.send(jellybeanBag);
//   console.log('Response Sent');
});

app.post('/beans/', (req, res, next) => {
  // logRequest('POST');
//   let queryData = '';
//   req.on('data', (data) => {
//     queryData += data;
//   });

//   req.on('end', () => {
    const body = req.body//JSON.parse(queryData);
    const beanName = body.name;
    if (jellybeanBag[beanName] || jellybeanBag[beanName] === 0) {
    //   return res.status(404).send('Bean with that name does not exist');
    //error middleware
        const err = new Error('Bean with that name does not exist');
        err.status = 404;
        return next(err);
    }
    const numberOfBeans = Number(body.number) || 0;
    jellybeanBag[beanName] = {
      number: numberOfBeans
    };
    res.send(jellybeanBag[beanName]);
    // console.log('Response Sent');
//   });
});

app.get('/beans/:beanName', (req, res, next) => {
  // logRequest('GET');
  res.send(req.bean);
//   console.log('Response Sent');
});


app.post('/beans/:beanName/add', (req, res, next) => {
  // logRequest('POST');
//   let queryData = '';
//   req.on('data', (data) => {
//     queryData += data;
//   });

//   req.on('end', () => {
    const numberOfBeans = Number(/*JSON.parse(queryData)*/req.body.number) || 0;
    jellybeanBag[beanName].number += numberOfBeans;
    res.send(req.bean);
    // console.log('Response Sent');
//   });
});

app.post('/beans/:beanName/remove', (req, res, next) => {
  // logRequest('POST');
//   let queryData = '';
//   req.on('data', (data) => {
//     queryData += data;
//   });

//   req.on('end', () => {
    const numberOfBeans = Number(/*JSON.parse(queryData)*/req.body.number) || 0;
    if (req.bean.number < numberOfBeans) {
    //   return res.status(400).send('Not enough beans in the jar to remove!');
    //error middleware
        const err = new Error('Not enough beans in the jar to remove!');
        err.status = 400;
        return next(err);
    }
    req.bean.number -= numberOfBeans;
    res.send(req.bean);
    // console.log('Response Sent');
//   });
});

app.delete('/beans/:beanName', (req, res, next) => {
  // logRequest('DELETE');
  req.bean = null;
  res.status(204).send();
//   console.log('Response Sent');
});

app.put('/beans/:beanName/name', (req, res, next) => {
  // logRequest('PUT');
//   let queryData = '';
//   req.on('data', (data) => {
//     queryData += data;
//   });

//   req.on('end', () => {
    const newName = /*JSON.parse(queryData)*/req.body.name;
    jellybeanBag[newName] = jellybeanBag[beanName];
    jellybeanBag[req.beanName] = null;
    res.send(jellybeanBag[newName]);
    // console.log('Response Sent');
//   });
});

// XI error middleware
app.use((err, req, res, next) => {
    const status = err.status || 500;
    res.status(status).send(err.message);
})
app.listen(PORT, () => {
  console.log(`Server is listening on port ${PORT}`);
});
